...
async function displayActors(){
    ...
    actors.forEach(element => {
        ...
        const nameContent = document.createTextNode(`Name: ${element.name}`);

        const rateDiv = document.createElement("div");
        const rateContent = document.createTextNode(`Rating: ${element.rating}`)

        const altNameDiv = document.createElement("div");
        let altNameContent;
        if(element.alternative_name=== null){
            altNameContent = document.createTextNode("Alternative name: No disponible")
        } else {
             altNameContent = document.createTextNode(`Alternative name: ${element.alternative_name}`)
        }

        const objectIdDiv = document.createElement("div");
        const objectIdContent = document.createTextNode(`ID: ${element.objectID}`)
